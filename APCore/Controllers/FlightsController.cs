...
namespace APCore.Controllers
{
    ...
    public class FlightsController : ControllerBase
    {
        ...
        {
            var result ="v2 "+ DateTime.Now.ToString("yyyyMMddHHmm");
        ...
        [HttpPost]
        [Authorize]
        [Route("api/flight/log/save/prop")]
        public async Task<IActionResult> SaveFlightProp(dynamic log)
        {
            var result = await _flightService.SaveFlightProp(log);
            if (!result.IsSuccess)
                return NotFound(result.Errors);
            return Ok(result);
        }
