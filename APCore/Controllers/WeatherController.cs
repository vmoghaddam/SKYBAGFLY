...
namespace APCore.Controllers
{
    ...
    public class WeatherController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetTAFADDS_DateHistoryByFDPIdFromArchive(int fdpId)
        {
            var _flights = await _flightService.GetFDPFlights(fdpId);
            var flights = (_flights.Data as List<AppCrewFlight>).OrderBy(q => q.STD).ToList();
            List<DateTime?> dts = flights.Select(q => q.STDDay).Distinct().ToList();
            var dt1 =(DateTime) dts.First();
            DateTime? dt2 = null;
            if (dts.Count > 1)
                dt2 = dts[1];
            var _stations = new List<string>();
            ...
            _stations = _stations.Distinct().ToList();
            var stations = string.Join(',', _stations);
            var result = await _weatherService.GetTAF_ADDS_FromArchive(stations, dt1,dt2);
            if (!result.IsSuccess)
                return NotFound(result.Errors);
            return Ok(result);
        ...
        public async Task<ActionResult> GetMETARADDS_DateHistoryFromArchive(int fdpId)
        {
            ...
            _stations = _stations.Distinct().ToList();
            var stations = string.Join(',', _stations);
            var result = await _weatherService.GetMETAR_ADDS_FromArchive(stations, dt1, dt2);
            if (!result.IsSuccess)
                return NotFound(result.Errors);
            return Ok(result);
        ...
        [Route("api/weather/metar/adds/archive/flight/{flt}")]
        public async Task<ActionResult> GetMETARADDS_DateHistoryFromArchiveByFlight(int flt)
        {
            //"2021-08-10T00:00:00+0430"
            var _flight = await _flightService.GetFlight(flt);
            if (!_flight.IsSuccess || _flight.Data == null)
                return NotFound(new List<string>() { "Flight  Not Found." });

            var flight  = (_flight.Data as AppLeg );

            var dt1 = (DateTime)flight.STDDay;
            ...
            _stations = _stations.Distinct().ToList();
            var stations = string.Join(',', _stations);
            var result = await _weatherService.GetMETAR_ADDS_FromArchive(stations, dt1, null);
            if (!result.IsSuccess)
                return NotFound(result.Errors);
            return Ok(result);
        ...
        [Route("api/weather/taf/adds/archive/flight/{flt}")]
        public async Task<ActionResult> GetTAFADDS_DateHistoryByFDPIdFromArchiveByFlight(int flt)
        {
            var _flight = await _flightService.GetFlight(flt);
            if (!_flight.IsSuccess || _flight.Data == null)
                return NotFound(new List<string>() { "Flight  Not Found." });
            ...
            _stations = _stations.Distinct().ToList();
            var stations = string.Join(',', _stations);
            var result = await _weatherService.GetTAF_ADDS_FromArchive(stations, dt1, null);
            if (!result.IsSuccess)
                return NotFound(result.Errors);
            return Ok(result);
