...
app.controller('epLogBookController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'authService', 'notificationService', '$route', '$window', '$q', '$http', function ($scope, $location, $routeParams, $rootScope, flightService, authService, notificationService, $route, $window, $q, $http) {
    $scope._IsJLVisible = $rootScope.userName.toLowerCase() == 'zahiri' || $rootScope.userName.toLowerCase() == 'hadadi';
   ...
   //  $scope.dt_to = new Date(2021, 7, 26);
    ...
    $scope.showFlight = function (item, items, index) {
        ...
        //}
      //  $scope.atctitle = 'AFP (✓)';
        db.GetSeenHistory({ FlightId: $scope.selectedFlight.FlightId, Type: 'ATC' }, function (item) {
            $scope.isMetarSeen = item != null ? $scope.atctitle = 'AFP (✓)' : $scope.atctitle = 'AFP';
        });
        db.GetSeenHistory({ FlightId: $scope.selectedFlight.FlightId, Type: 'ATL' }, function (item) {
            $scope.isMetarSeen = item != null ? $scope.atltitle = 'ATL (✓)' : $scope.atltitle = 'ATL';
        });
    ...
    $scope.isTafSeen = false;
    $scope.isMetarSeen = false;
    $scope.isNotamSeen = false;
    $scope.folderClick = function (g) {
        ...
        $scope.isMetarSeen = false;
        $scope.isNotamSeen = false;
        db.GetSeenHistory({ FlightId: $scope.selectedGroup.FDPId, Type: 'TAF' }, function (item) {
            $scope.isTafSeen = item != null;
        });
        db.GetSeenHistory({ FlightId: $scope.selectedGroup.FDPId, Type: 'METAR' }, function (item) {
            $scope.isMetarSeen = item != null;
        });
        db.GetSeenHistory({ FlightId: $scope.selectedGroup.FDPId, Type: 'NOTAM' }, function (item) {
            $scope.isNotamSeen = item != null;
        });
        ...
        if ($rootScope.getOnlineStatus()) {
            flightService.checkInternet(function (st) {
                if (st) {
                    ...
                    $rootScope.$broadcast('InitJLTabanAdd', data);
    ...
    $scope.showTAF = function (g) {
        ...
        db.DeleteSeenHistory(g.FDPId, 'TAF', function () {
            db.AddSeenHistory({ FlightId: g.FDPId, Type: 'TAF' }, function () { $scope.isTafSeen = true; });
          });
        $rootScope.$broadcast('InitTAF', g);
    };
    $scope.showMETAR = function (g) {
        db.DeleteSeenHistory(g.FDPId, 'Metar', function () {
            db.AddSeenHistory({ FlightId: g.FDPId, Type: 'METAR' }, function () { $scope.isMetarSeen = true; });
        });
        $rootScope.$broadcast('InitMETAR', g);
    };
    $scope.showNOTAM = function (g) {
        db.DeleteSeenHistory(g.FDPId, 'Notam', function () {
            db.AddSeenHistory({ FlightId: g.FDPId, Type: 'NOTAM' }, function () { $scope.isNotamSeen = true; });
        });
        ...
        flightService.epGetCrewFlights(_d1, _d2).then(function (response) {
            ...
            if (response.IsSuccess) {
                ...
                flightService.epGetCrewDuties(GlobalUserId, _d1, _d2).then(function (response2) {
                    ...
                    console.log('    *******************************     GROUPED', $scope.groupedFlights);
    ...
    $scope.btn_to = {
        ...
        onClick: function (e) {
            ...
            //    return;
            //}
            var _fdp = Enumerable.From($scope.groupedFlights).Where('$.FDPId==' + $scope.selectedFlight.FDPId).FirstOrDefault();

            var data = { FlightId: $scope.selectedFlight.FlightId, fdp: _fdp, station: $scope.selectedFlight.FromAirportIATA };
    ...
    $scope.btn_lnd = {
        ...
        onClick: function (e) {
           ...
           // throw "Too big";
            //
            var _fdp = Enumerable.From($scope.groupedFlights).Where('$.FDPId==' + $scope.selectedFlight.FDPId).FirstOrDefault();
            var data = { FlightId: $scope.selectedFlight.FlightId, fdp: _fdp, station: $scope.selectedFlight.ToAirportIATA  };
    ...
    $scope.btn_atc = {
        ...
        onClick: function (e) {
            if ($scope.selectedFlight.ATCPlan) {
                ...
                db.DeleteSeenHistory($scope.selectedFlight.FlightId, 'ATC', function () {
                    db.AddSeenHistory({ FlightId: $scope.selectedFlight.FlightId, Type: 'ATC' }, function () { $scope.atctitle = 'AFP (✓)'; });
                });
            }
